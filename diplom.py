...
def check_reset_with_llm(query: str, chat_history: list) -> tuple:
    ...
    # Если последнее сообщение бота - это ответ на сброс, игнорируем новые попытки сброса
    if chat_history and len(chat_history) > 0:
        ...
        if last_bot_reply and "Давайте начнём поиск для другого автомобиля" in last_bot_reply:
            # Только что сбросили поиск, не нужно сбрасывать снова
            return False, ""
        
    # ПО КЛЮЧЕВЫМ СЛОВАМ
    reset_phrases = ["начать новый поиск", "новый поиск", "начать заново", "другая машина"]
    ...
    context = " ".join([u for u, b in chat_history[-3:] if u and isinstance(u, str)])

    prompt = f"""
Ты — ассистент интернет-магазина автозапчастей. Определи, хочет ли пользователь начать новый поиск для другого автомобиля.

Правила:
- Верни "ДА", если пользователь хочет найти запчасть на другую машину, сбросить текущий поиск, начать заново.
- Верни "НЕТ" в остальных случаях.

ВАЖНО: Если пользователь вводит год автомобиля (например "2020") или объём двигателя (например "1.4"), это НЕ запрос на новый поиск. Верни "НЕТ".

Примеры фраз, которые означают "ДА":
- "другая машина", "другое авто", "на другую машину"
- "хочу найти на другую машину", "поиск для другого авто"
- "сбросить", "начать заново", "новый поиск", "давай дальше"
- "поехали дальше", "дальше", "ещё"

Примеры фраз, которые означают "НЕТ":
- обычный запрос запчасти: "нужны колодки", "масляный фильтр"
- уточнение параметров автомобиля: "2012", "1.4", "2020 1.6"
...
"""
    try:
        ...
        print(f"Ошибка check_reset_with_llm: {e}")
        return False, ""
